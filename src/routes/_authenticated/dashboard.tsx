...
function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="border-b border-border bg-[#FAF9F6]">
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="min-w-28 rounded-2xl border border-slate-200/80 bg-white px-5 py-3.5 shadow-sm text-center">
      <p className="font-display text-2xl font-bold leading-none text-slate-900">{value}</p>
      <p className="mt-1.5 text-[11px] font-bold tracking-widest text-slate-400 uppercase">{label}</p>
    </div>
  );
}
...
function FormCard({
  ...
  return (
    <motion.article
      ...
      className="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-slate-200/90 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-sky-200"
    >
      <span
        aria-hidden
        className={cn(
          "absolute inset-x-0 top-0 h-1 origin-left transition-transform duration-300",
          form.published ? "bg-sky-600" : "bg-slate-200",
        )}
      />
      <div>
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-2">
          <Link
            to="/builder/$id"
            params={{ id: form.id }}
            className="font-display text-lg font-bold leading-snug hover:text-sky-600 transition-colors"
          >
            {form.title}
          </Link>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                className="shrink-0 rounded-lg p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all"
      ...
      <div className="mt-6 flex items-center justify-between gap-2 border-t pt-4 border-slate-100">
        ...
        <Link
          to="/responses/$id"
          params={{ id: form.id }}
          className="text-xs font-semibold text-slate-500 hover:text-sky-600 hover:underline"
        >
      ...
      <DialogContent className="sm:max-w-md rounded-2xl">
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-600">Live Form Link</label>
          ...
          {/* QR Code */}
          <div className="space-y-1.5 text-center p-4 border border-slate-100 rounded-xl bg-slate-50">
            <label className="text-xs font-semibold text-slate-700 flex items-center justify-center gap-1.5 mb-2">
              <QrCode className="h-4 w-4 text-sky-600" /> Scannable Poster QR Code
            </label>
          ...
          {/* Embed snippet */}
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-600">Website Embed Code (Iframe)</label>
            <div className="flex gap-2">
...
function EmptyState({ hasForms, onCreate }: { hasForms: boolean; onCreate: () => void }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      className="grid-paper rounded-3xl border border-dashed border-slate-300 p-12 text-center bg-white shadow-sm"
    >
      <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-sky-50 text-sky-600 mb-4">
